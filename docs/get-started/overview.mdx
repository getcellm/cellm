...
- `=PROMPT()` behaves like normal Excel functions. Prompts can reference other cells, re-run when inputs change, and outputs can be filtered, charted, and used in other Excel formulas.
- Outputs model response in a single cell, spills multiple values into a row or column, or spills multiple values into a table (a rectangular group of cells).
