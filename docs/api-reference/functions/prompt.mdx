...
- `=PROMPT.TOROW()` will spill multiple output values (if any) into cells to the right.
- `=PROMPT.TOCOLUMN()` will spill multiple output values (if any) into cells below.
- `=PROMPT.TORANGE()` will spill multiple output values (if any) into a range.